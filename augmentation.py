...
class DataAugmentation:
    ...
    def randomRotation(image,mode=Image.BICUBIC):
        ...
        random_angle = angle[n]
        return image.rotate(random_angle,mode)

    @staticmethod
    def randomFlip_1(image):
        #图像翻转（类似于镜像，镜子中的自己）
    ...
    def CenterCrop(image):
        ...
        image_width = image.size[0]
        image_height = image.size[1]
        new_image_width=0.8*image_width
        new_image_height=0.8*image_height
    ...
    def randomColor(image):
        ...
        random_factor=np.random.randint(7,15)*1.0/10.0 # 随机因子
        # print(random_factor)
        color_image=ImageEnhance.Color(image).enhance(random_factor) # 调整图像的饱和度
        # 亮度增强,增强因子为0.0将产生黑色图像；为1.0将保持原始图像。
        random_factor=np.random.randint(7,15)*1.0/10.0
        # print(random_factor)
        brightness_image=ImageEnhance.Brightness(color_image).enhance(random_factor) # 调整图像的亮度
        random_factor = np.random.randint(7, 15) *1.0/ 10.0  # 随机因1子
        # print(random_factor)
    ...
    def randomGaussian(image,mean=0,sigma=0.2):
        ...
        :return:
        """
        def gaussianNoisy(im,mean=0,sigma=0.2):
            """
            :param im:单通道图像
            :param mean:
            :param sigma:
